...
Returns the outline (table of contents) of an act — its chapters, sections, articles and annexes — each with a character offset into the document's plain text. Identify the act by celex_id, eli, or oj_ref (exactly one). Use it as a map for targeted reading: read an article's offset from the outline, then call eurlex_fetch(celex_id, format:"plain", offset, max_chars) with that offset to jump straight to that article instead of paging from the top of a long act. Pass the SAME language to the follow-up fetch and keep format:"plain" — offsets are specific to the chosen language and to plain (tag-stripped) text. Heading recognition covers English, German and French. For case-law documents (CELEX sector 6, e.g. CJEU judgments) the outline also lists each numbered judgment paragraph as "Paragraph N" (level 4); paragraph detection is language-independent (it keys on the number, not heading words), so you can grab the offset of paragraph 72 of a judgment and jump straight to it.
...
7. A specific article of a long act (or a numbered paragraph of a CJEU judgment)? → eurlex_structure first to get its offset, then eurlex_fetch(format:"plain", offset) to jump straight to it. For case law (sector 6) the outline lists each numbered paragraph as "Paragraph N", so "read paragraph 72 of a judgment" = eurlex_structure to get its offset, then eurlex_fetch(offset)
