...
export async function handleEurlexStructure(input: {
  ...
  try {
    ...
    // Sector 6 = case law → CJEU judgments carry bare-number paragraph markers.
    const isCaseLaw = celexId.startsWith('6');
    const { entries, total, truncated } = parseOutline(plain, OUTLINE_MAX_ENTRIES, {
      numberedParagraphs: isCaseLaw,
    });
    ...
    if (entries.length === 0) {
      result.note = isCaseLaw
        ? 'No numbered paragraphs or structural headings were detected. The document may be ' +
          'a short order/decision or available only as PDF. The full text is still readable ' +
          'with eurlex_fetch.'
